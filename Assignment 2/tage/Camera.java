...
public class Camera
{
	...
	/** Yaw: rotates camera on Y axis */
	public void yaw(float scalar, MyGame game){
		game.cam = (game.engine.getRenderSystem().getViewport("MAIN").getCamera());
            Vector3f oldRight = game.cam.getU();
            Vector3f oldFwd = game.cam.getN(); 
            Vector3f oldUp = game.cam.getV();
            Matrix4f rotationAroundCamUp = new Matrix4f().rotation(scalar, oldUp); 
            Vector3f newForward = rotationAroundCamUp.transformDirection(oldFwd);
            Vector3f newRight = rotationAroundCamUp.transformDirection(oldRight);
            game.cam.setN(newForward);
            game.cam.setU(newRight);
	}

	/** Pitch: rotates object on X axis */
	public void pitch(float scalar, MyGame game) {
		game.cam = (game.engine.getRenderSystem().getViewport("MAIN").getCamera());
		Vector3f oldRight = game.cam.getU(); // Right vector
		Vector3f oldFwd = game.cam.getN(); // Forward vector
		Vector3f oldUp = game.cam.getV();
		Matrix4f rotationAroundCamRight = new Matrix4f().rotation(-scalar, oldRight);
		Vector3f newForward = rotationAroundCamRight.transformDirection(oldFwd);
		Vector3f newUp = rotationAroundCamRight.transformDirection(oldUp);
		game.cam.setN(newForward);
		game.cam.setV(newUp);
	}
